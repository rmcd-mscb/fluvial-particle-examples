...
First, let's get the default settings template. The `get_settings_template()` function returns a complete settings file with all available options:
...
Now customize the template for our example data. We need to update:

- File paths to point to our example grids
...
settings_content = template.replace(
    'file_name_2d = "./path/to/your/mesh_2d.vts"',
    f'file_name_2d = "{file_2d}"'
).replace(
    'file_name_3d = "./path/to/your/mesh_3d.vts"',
    f'file_name_3d = "{file_3d}"'
).replace(
...
You can also generate the template from the command line:
```bash
fluvial_particle --init
```
This creates `user_options.py` in the current directory.
:::
